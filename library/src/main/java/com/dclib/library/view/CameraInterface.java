...
import com.dclib.library.CameraUtil;
import com.dclib.library.listener.ErrorListener;
import com.dclib.library.util.AngleUtil;
import com.dclib.library.util.DeviceUtil;
...
public class CameraInterface {
    ...
    private synchronized void safeOpenCamera(int cameraID) {
        try {
            ...
            Log.e(TAG, "safe open camera exception " + throwable.getMessage());
        }
    }
    ...
    public synchronized void doStartPreview(SurfaceHolder holder, float screenProp) {
        ...
        if (mCamera != null) {
            try {
                ...
                Log.i(TAG, "start preview exception " + e.getMessage());
                if (this.errorListener != null) {
    ...
    public synchronized void doStopPreview() {
        if (null != mCamera) {
            try {
                ...
                Log.i(TAG, "stop preview exception " + e.getMessage());
            }
        }
    }
    ...
    void doDestroyCamera() {
        ...
        if (null != mCamera) {
            try {
                ...
                Log.i(TAG, "destroy camera exception " + e.getMessage());
            }
        } else {
            Log.i(TAG, "doDestroyCamera camera is null");
        }
    }
    ...
    public synchronized void startRecord(Context context, Surface surface, float screenProp) {
        ...
        try {
            ...
            videoFileAbsPath = FileUtil.generateVideoFilePath(context);
